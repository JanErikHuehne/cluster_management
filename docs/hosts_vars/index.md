...
e.g. `host_vars/tuwzc1n-thalamus.yml`.

Anything that is identical across nodes belongs in `group_vars/` instead.
Only genuinely host-specific facts — hardware topology and the node's own AD
...
```bash
slurmd -C
# NodeName=tuwzc1n-thalamus CPUs=96 Boards=1 SocketsPerBoard=2 \
...
`Low RealMemory` if it finds less — but never complains about finding more.
...
`CoreSpecCount=`. Counted in **cores**, not CPUs — with `slurm_threads: 2`,
...
!!! note
    ...
    `slurm_mem_spec_limit` for the same purpose — that double-reserves and
...
characters, so longer hostnames are truncated:

| Node                | Machine account     |
|---------------------|---------------------|
| `tuwzc1n-insula`    | `TUWZC1N-INSULA$`   |
| `tuwzc1n-thalamus`  | `TUWZC1N-THALAMU$`  |

Any hostname over 8 characters is truncated, given the 7-character
`TUWZC1N-` prefix. Collisions may also cause the join tool to append digits.
...
```yaml
# host_vars/tuwzc1n-thalamus.yml
slurm_cpus: 96
slurm_sockets: 2
slurm_cores: 24
slurm_threads: 2
...
sssd_sasl_authid: TUWZC1N-THALAMU$
...
```yaml
# host_vars/tuwzc1n-habenula.yml
slurm_cpus: 112
slurm_sockets: 2
slurm_cores: 28
slurm_threads: 2
...
sssd_sasl_authid: TUWZC1N-HABENUL$
